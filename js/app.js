...
function scheduleSimulation(){
  ...
  anchored.forEach(t=>{
    const ids=engineResolveResources(t);
    // Never use t.start from a previous Apply — always place from today
    const start=GANTT_TODAY;
...
function computeWarnings(){
  ...
  Object.entries(occ).forEach(([resId,days])=>{
    ...
    const futureDays=Object.keys(days).map(Number).filter(d=>d>=GANTT_TODAY);
    const lastDay=futureDays.length?Math.max(...futureDays):0;
    Object.entries(days).forEach(([d,h])=>{
      const day=+d;
      if(day<GANTT_TODAY) return; // ignore past days
      const cap=dayCapacity(resId,day).hours;
      ...
      } else if(h>0.01 && h<cap-0.01 && day!==lastDay){
