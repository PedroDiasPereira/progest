...
function applySimulation(sim){
  ...
  Object.entries(sim.preview).forEach(([taskId,p])=>{
    ...
    // Clear cached render position so the next simulation compares against
    // the newly-written t.start, not the stale pre-apply value.
...
(async function(){
  ...
  // Inject version badge next to sync-status
  (()=>{
    const _ss=document.getElementById('sync-status');
    if(_ss&&!document.getElementById('app-ver-badge')){
      const _vb=document.createElement('span');
      _vb.id='app-ver-badge';
      _vb.textContent='v'+APP_VERSION;
      _vb.title='app.js build version';
      _vb.style.cssText='font-size:9px;font-weight:700;color:var(--fg3);opacity:.6;letter-spacing:.04em;margin-left:4px;user-select:none';
      _ss.insertAdjacentElement('afterend',_vb);
    }
  })();
