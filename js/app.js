...
function getRes(id){
  ...
  // Always derive initials from the current name — avoids stale .initials left over
  // from a previous name (e.g. resource renamed via a path that didn't recompute it).
  if(r&&r.name){ const fresh=_getInitials(r.name); if(fresh&&r.initials!==fresh) r.initials=fresh; }
  return r;
}
...
// STATE
const S = {
  page:'gantt', viewBy:'resource', zoom:'week', offset:0, _wlView:'week', _wlOffset:0, _sortBy:'chrono', _noGrpSplit:false, _activeProjIds:[],
...
function nav(p){
  ...
  if(p==='overview'){
    // renderOverview() handles all filter restoration internally
    // (uses _ovProjRestore / _ovTypeRestore / _ovSearchRestore + sessionStorage fallback)
    renderOverview();
  }
...
function toggleSF(s){
  ...
  buildSFChips(); _saveSessionState(); renderGantt();
}
...
function toggleVBPanel(){
  if(S.viewBy==='project'){
    // Project view: open multi-select panel for projects (same pattern as teams/resources)
    const p=document.getElementById('vb-sel-panel');
    if(p.style.display==='none'||!p.style.display){
      p.style.display='block';
      const items=PROJECTS.filter(pr=>pr.status!=='on_hold'&&pr.status!=='cancelled').map(pr=>({id:pr.id,label:pr.name,col:pr.color||'var(--acc)'}));
      const _vbAllChecked=(i)=>S._activeProjIds.length===0||S._activeProjIds.includes(i.id);
      const _getInitials=name=>name.split(' ').map(w=>w[0]||'').join('').toUpperCase().slice(0,2);
      const _renderItems=(filter)=>{
        ...
        return filtered.map(i=>`<label style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:6px;cursor:pointer;font-size:12px;color:var(--fg0);transition:background .1s" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background='transparent'">
          <input type="checkbox" value="${i.id}" ${_vbAllChecked(i)?'checked':''} onchange="toggleVBFProj('${i.id}',this.checked)" style="accent-color:${i.col};width:14px;height:14px;flex-shrink:0">
          <span style="width:24px;height:24px;border-radius:6px;background:${i.col};display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;flex-shrink:0">${_getInitials(i.label)}</span>
          <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${i.label}</span>
        </label>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--fg3);text-align:center">No results</div>';
      };
      window._renderItems=_renderItems;
      window._vbProjSelectAll=()=>{ S._activeProjIds=[]; renderGantt(); document.getElementById('vb-sel-panel').style.display='none'; };
      window._vbProjSelectNone=()=>{ S._activeProjIds=['__none__']; renderGantt(); document.getElementById('vb-sel-panel').style.display='none'; };
      window._vbProjSearch=(v)=>{ document.getElementById('vb-list').innerHTML=window._renderItems(v); };
      p.innerHTML=
        ...
        +'oninput="window._vbProjSearch(this.value)" onkeydown="window._onVBKey(event)">'
        +'</div>'
        +'<div style="display:flex;gap:6px;padding:5px 8px;border-bottom:1px solid var(--bd2)">'
        +'<button onclick="window._vbProjSelectAll()" style="flex:1;font-size:10px;padding:3px;border-radius:4px;border:1px solid var(--bd2);background:var(--bg3);color:var(--fg2);cursor:pointer;font-family:var(--font)">&#10003; All</button>'
        +'<button onclick="window._vbProjSelectNone()" style="flex:1;font-size:10px;padding:3px;border-radius:4px;border:1px solid var(--bd2);background:var(--bg3);color:var(--fg2);cursor:pointer;font-family:var(--font)">&#10005; None</button>'
        +'</div>'
        +'<div id="vb-list" style="padding:4px">'+_renderItems('')+'</div>';
      window._onVBKey=(e)=>{
        if(e.key!=='Enter') return;
        const filtered=window._vbFilteredItems||[];
        if(filtered.length===1){ S._activeProjIds=[filtered[0].id]; renderGantt(); document.getElementById('vb-sel-panel').style.display='none'; }
      };
...
function toggleVBFProj(id, checked){
  if(checked){
    if(!S._activeProjIds.includes(id)) S._activeProjIds.push(id);
    const allProjIds=PROJECTS.filter(p=>p.status!=='on_hold'&&p.status!=='cancelled').map(p=>p.id);
    if(allProjIds.every(x=>S._activeProjIds.includes(x))) S._activeProjIds=[];
  } else {
    if(!S._activeProjIds.length){
      S._activeProjIds=PROJECTS.filter(p=>p.status!=='on_hold'&&p.status!=='cancelled').map(p=>p.id).filter(x=>x!==id);
    } else {
      S._activeProjIds=S._activeProjIds.filter(x=>x!==id);
    }
    if(!S._activeProjIds.length) S._activeProjIds=['__none__'];
  }
  // Sync all checkboxes in the panel without rebuilding it (keeps panel open for multi-select)
  const panel=document.getElementById('vb-sel-panel');
  if(panel){
    panel.querySelectorAll('input[type=checkbox]').forEach(cb=>{
      const _allChecked=S._activeProjIds.length===0||S._activeProjIds.includes(cb.value);
      cb.checked=_allChecked;
    });
  }
  // Update button label
  const _sb=document.getElementById('vb-sel-btn');
  if(_sb){ const _n=S._activeProjIds.filter(x=>x!=='__none__').length; _sb.textContent=(_n>0?`${_n} selected`:'Select…')+' ▾'; }
  _saveSessionState();
  // Defer renderGantt so the click event finishes propagating before reflow
  // (prevents the document click listener from closing the panel mid-selection)
...
function visTasks(){
  ...
  let tasks=TASKS.filter(t=>{
    ...
    // Project filter: _activeProjIds (multi) when in project view, else _activeProjId (single)
    if(S.viewBy==='project'&&S._activeProjIds?.length&&!S._activeProjIds.includes(t.projId||null)) return false;
    else if(S.viewBy!=='project'&&S._activeProjId && (t.projId||null) !== S._activeProjId) return false;
  ...
  // Sort by group as primary key — unless _noGrpSplit is active (user wants pure chrono)
  if(!S._noGrpSplit){
...
window._toggleGrpSplit=()=>{
  S._noGrpSplit=!S._noGrpSplit;
  const btn=document.getElementById('btn-grp-split');
  if(btn){
    btn.style.opacity=S._noGrpSplit?'1':'.55';
    btn.style.borderColor=S._noGrpSplit?'var(--acc)':'';
    btn.style.color=S._noGrpSplit?'var(--acc)':'';
  }
  renderGantt();
};
...
function buildGRows(tasks){
  ...
  // When S._noGrpSplit: skip group header rows, pass grpColor as subtle hint on each task row.
  function _pushWithGroupHeaders(taskList, addRowFn){
    if(S._noGrpSplit){
      taskList.forEach(t=>{
        // Pass grpColor so the row renderer can show a subtle dot in the Group column
        t._grpColor=_GRP_COLORS[t.group||'']||null;
        addRowFn(t);
      });
      return;
    }
...
function getEffectiveMinStart(t){
  ...
  if(dep.depType==='group'&&dep.depGroup){
    const groupTasks=TASKS.filter(t=>t.group===dep.depGroup&&(!dep.projId||t.projId===dep.projId)&&t.id!==dep.id&&t.status!=='cancelled');
...
function renderOverview(){
  ...
  if(!el) return;
  // Resolve filter values: synchronous restore-vars (F5) take priority, then sessionStorage
  let _ovRestoredProj='', _ovRestoredType='', _ovRestoredSearch='';
  try{
    const _sk='pg_ui_'+(S_USER?.resId||'');
    const _sd=JSON.parse(sessionStorage.getItem(_sk)||'{}');
    _ovRestoredProj  = window._ovProjRestore  || _sd._ovProj  || '';
    _ovRestoredType  = window._ovTypeRestore  || _sd._ovType  || '';
    _ovRestoredSearch= window._ovSearchRestore|| _sd._ovSearch|| '';
    window._ovProjRestore=''; window._ovTypeRestore=''; window._ovSearchRestore='';
  }catch(e){}
  // ov-search (input — safe to set value directly)
  const _os=document.getElementById('ov-search');
  if(_os && _ovRestoredSearch) _os.value=_ovRestoredSearch;
  // ov-type (static options — rebuild with selected attribute to bypass hidden-element quirk)
  const _ot=document.getElementById('ov-type');
  if(_ot){
    const _typeOpts=[['','Tasks + Milestones'],['tasks','Tasks only'],['milestones','Milestones only']];
    _ot.innerHTML=_typeOpts.map(([v,l])=>`<option value="${v}"${v===_ovRestoredType?' selected':''}>${l}</option>`).join('');
  }
  // ov-proj (dynamic options — rebuild with selected attribute)
  const ps=document.getElementById('ov-proj');
  if(ps){
    ps.innerHTML='<option value="">All projects</option>'+PROJECTS.map(p=>`<option value="${p.id}"${p.id===_ovRestoredProj?' selected':''}>${p.name}</option>`).join('');
  }
  const projId=_ovRestoredProj;
  const typeF=_ovRestoredType;
  const q=(_ovRestoredSearch).toLowerCase();
...
function renderGantt(){
  ...
  {const _sb=document.getElementById('vb-sel-btn');if(_sb){ _sb.style.display=''; const _nSel=S._activeProjIds.length&&S.viewBy==='project'?S._activeProjIds.filter(x=>x!=='__none__').length:S.vbFilter.length; _sb.textContent=(_nSel>0?`${_nSel} selected`:'Select…')+' ▾'; }}
  ...
  // btn-grp-split: only visible in project view; reflect active state
  {const _bg=document.getElementById('btn-grp-split');if(_bg){_bg.style.display='';_bg.style.opacity=S._noGrpSplit?'1':'.55';_bg.style.borderColor=S._noGrpSplit?'var(--acc)':'';_bg.style.color=S._noGrpSplit?'var(--acc)':'';}}
  ...
  // Populate project selector — use S._activeProjId as fallback (restored from session)
  const _pSel=document.getElementById('proj-selector');
  if(_pSel){
    const _cur=_pSel.value||S._activeProjId||'';
  ...
  // Restore g-search and selectedTags from sessionStorage if DOM is empty (after F5)
  try{
    const _sk='pg_ui_'+(S_USER?.resId||'');
    const _ss=sessionStorage.getItem(_sk);
    if(_ss){
      const _sd=JSON.parse(_ss);
      const _gs=document.getElementById('g-search');
      if(_gs&&!_gs.value&&_sd._ganttSearch) _gs.value=_sd._ganttSearch;
      if(_sd._ganttTags?.length&&!(S.selectedTags?.length)) S.selectedTags=_sd._ganttTags;
    }
  }catch(e){}
  ...
  <tr>
    ...
    ${cols.map(c=>{
      ...
      const _mProjOk=(m)=>{ if(S.viewBy==='project'){ if(!S._activeProjIds.length) return true; return S._activeProjIds.includes(m.projId||(TASKS.find(t=>(m.taskIds||[]).includes(t.id))?.projId)||null); } return !S._activeProjId||(m.projId?m.projId===S._activeProjId:(m.taskIds||[]).some(id=>TASKS.find(t=>t.id===id&&t.projId===S._activeProjId))); };
      const mHere=MILESTONES.filter(m=>m.dayIdx===c.idx&&_mProjOk(m));
  ...
  rows.forEach((row,ri)=>{
    ...
    H+=`<tr data-tid="${t.id}" style="${rowStyle}">
      <td class="stg" style="width:50px;max-width:50px;height:36px;text-align:center;vertical-align:middle;font-size:9px;font-weight:700;font-family:var(--mono);color:var(--fg2);padding:0 2px">${S._noGrpSplit?(t._grpColor?`<span title="${t.group||''}" style="display:inline-block;width:5px;height:5px;border-radius:50%;background:${t._grpColor};flex-shrink:0"></span>`:''):(t.group||'')}</td>
      <td class="stn" style="padding:2px 6px;width:240px;max-width:240px;height:36px;overflow:hidden;vertical-align:middle;${S._noGrpSplit&&t._grpColor?'border-left:2px solid '+t._grpColor+'55;padding-left:5px;':''}">
        <div style="display:flex;align-items:center;gap:4px">
          ...
          <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;font-size:11px;color:${isDone?'var(--fg2)':'var(--fg0)'};flex:1;min-width:0" onclick="openEditTask('${t.id}')" title="${t.name}">
            ...
            ${t.name}${(()=>{
              ...
              const _lines=[];
              if(t.projId&&(S.viewBy!=='project'?!S._activeProjId:S._activeProjIds.length!==1)){const _pr=PROJECTS.find(p=>p.id===t.projId);if(_pr)_lines.push('<span style="font-size:8px;padding:1px 4px;border-radius:3px;background:'+(_pr.color||'var(--acc)')+'22;color:'+(_pr.color||'var(--acc)')+';display:inline-block;margin-top:1px">'+_pr.name+'</span>');}
...
function engineMinStart(task, simPreview){
  ...
  // Helper: prefer the in-progress simulation preview for a dependency (if already
  // processed in this same scheduleSimulation() pass) over the stale TASKS.start/_sched
  // persisted from a previous Apply. Without this, simulations don't converge in one
  // pass when task B depends on task A and A is scheduled after B is read — each Apply
  // only "reveals" the next layer, requiring several Simulate→Apply cycles to settle.
  const _depEnd=(dep)=>{
    if(simPreview&&simPreview[dep.id]){
      const sb=simPreview[dep.id].schedByRes||{};
      const allDays=Object.values(sb).flatMap(m=>Object.keys(m).map(Number));
      if(allDays.length) return Math.max(...allDays);
    }
    return dep.start?tEnd(dep):0;
  };
  if(task.depType==='task'){
    ...
    dids.forEach(did=>{
      ...
      const depEnd=_depEnd(dep);
      if(depEnd>0) minStart=Math.max(minStart, nextWorkDay(depEnd+1));
    });
  }
  if(task.depType==='group'&&task.depGroup){
    ...
    const lastEnd=groupTasks.reduce((m,t)=>{ const e=_depEnd(t); return e>m?e:m; }, 0);
...
function scheduleSimulation(){
  ...
  anchored.forEach(t=>{
    ...
    const start=engineMinStart(t, preview);
    if(t.simultaneous && ids.length>1){
      ...
      const id=ids[0];
      const _ancStart=nextWorkDay(start);
      const schedByRes={};
      if(t.timeMode==='test'){
        const _dur=t.dur||1;
        const _hpd=t.resHours?.[id]>0&&_dur>0?Math.round(t.resHours[id]/_dur*100)/100:0;
        const m={};
        let day=_ancStart, placed=0, iters=0;
        while(placed<_dur&&iters++<500){ if(isNW(day,t.id,id)){day++;continue;} m[day]=_hpd; placed++; day++; }
        _seedOcc(id,m,t.id); schedByRes[id]=m;
        const _coIds=(t.coResIds||[]).filter(Boolean);
        _coIds.forEach(coId=>{
          const _coH=t.resHours?.[coId]||0; if(_coH<=0) return;
          const _coHpd=Math.round(_coH/_dur*100)/100;
          const coM={};
          let cday=_ancStart, cplaced=0, citers=0;
          while(cplaced<_dur&&citers++<500){ if(isNW(cday,t.id,coId)){cday++;continue;} coM[cday]=_coHpd; cplaced++; cday++; }
          _seedOcc(coId,coM,t.id); schedByRes[coId]=coM;
        });
        const _sDays=Object.keys(m).map(Number); 
        preview[t.id]={start:_sDays.length?Math.min(..._sDays):_ancStart, schedByRes};
      } else {
        const r=enginePlaceResource(t, id, _ancStart, occByRes[id]||{});
        ...
        coIds.forEach(coId=>{
          ...
          const coR=enginePlaceResource({...t,resId:coId,hours:coHours}, coId, _ancStart, occByRes[coId]||{});
          const coM={}; coR.segs.forEach(s=>coM[s.day]=(coM[s.day]||0)+s.hours);
          _seedOcc(coId,coM,t.id); schedByRes[coId]=coM;
        });
        preview[t.id]={start:r.segs[0]?.day||_ancStart, schedByRes};
      }
    }
  });
  ...
  todo.forEach(t=>{
    const ids=engineResolveResources(t);
    const minStart=engineMinStart(t, preview);
    ...
    if(t.assignType==='team' && t.effort!=='perHead' && ids.length>1){
      ...
      if(t.timeMode==='test'){
        // Test tasks: distribute resHours/dur per day for exactly dur work-days,
        // same logic as computeAllSchedules. Don't call enginePlaceResource.
        const _dur=t.dur||1;
        const _hpd=t.resHours?.[id]>0&&_dur>0?Math.round(t.resHours[id]/_dur*100)/100:0;
        const m={};
        let day=ffd, placed=0, iters=0;
        while(placed<_dur&&iters++<500){
          if(isNW(day,t.id,id)){day++;continue;}
          m[day]=_hpd; placed++; day++;
        }
        _seedOcc(id,m,t.id);
        schedByRes[id]=m;
        const startDay=Object.keys(m).map(Number).sort((a,b)=>a-b)[0]||ffd;
        // Seed co-resources with same pattern
        const coIds=(t.coResIds||[]).filter(Boolean);
        coIds.forEach(coId=>{
          const _coH=t.resHours?.[coId]||0; if(_coH<=0) return;
          const _coHpd=Math.round(_coH/_dur*100)/100;
          ...
          const coM={};
          let cday=coFfd, cplaced=0, citers=0;
          while(cplaced<_dur&&citers++<500){
            if(isNW(cday,t.id,coId)){cday++;continue;}
            coM[cday]=_coHpd; cplaced++; cday++;
          }
          _seedOcc(coId,coM,t.id);
          schedByRes[coId]=coM;
        });
        preview[t.id]={start:startDay, schedByRes};
      } else {
...
function applySimulation(sim){
  ...
  Object.entries(sim.preview).forEach(([taskId,p])=>{
    ...
    // recompute dur from sched span — but NOT for 'test' tasks, whose dur is
    // a fixed user-defined number of calendar days, not derived from scheduled hours
    if(t.timeMode!=='test'){
...
const WARN_META={
  ...
  VACATION_CONFLICT:{icon:'🏖',label:'Scheduled during time off',  col:'var(--danger)'}
};
...
function computeWarnings(){
  ...
  TASKS.forEach(t=>{
    ...
    if((tHours(t)??0)<=0) return; // no hours — cannot fill capacity
    if(t.sched) return; // already placed by simulation — not a fill candidate
    const isDoing=t.status==='doing'||t.status==='paused';
    const hasDep=t.depType==='task'&&([...(t.depIds||[]),t.depId].filter(Boolean).length>0);
    if(isDoing&&!hasDep) return;
  ...
  window._dbgEligibleByRes=_eligibleByRes;
  ...
  // 7 — Resource scheduled on a vacation/time-off day
  Object.entries(occ).forEach(([resId,days])=>{
    const res=getRes(resId); if(!res) return;
    const rd=RES_DAYS.get(resId);
    if(!rd||!rd.disabled||!rd.disabled.size) return;
    Object.entries(days).forEach(([d,h])=>{
      if(!(h>0.01)) return;
      const day=+d;
      if(day<GANTT_TODAY) return;
      if(!rd.disabled.has(d)&&!rd.disabled.has(String(day))) return;
      // Find which task(s) put hours on this resource/day, to report names and respect TASK_DAYS overrides
      const culprits=TASKS.filter(t=>{
        if(t.status==='done'||t.status==='cancelled') return false;
        const own=(t.resId===resId&&t._sched&&t._sched[d]>0);
        const co=(t._schedCo&&t._schedCo[resId]&&t._schedCo[resId][d]>0);
        if(!own&&!co) return false;
        const td=TASK_DAYS.get(t.id);
        if(td&&td.enabled&&(td.enabled.has(d)||td.enabled.has(String(day)))) return false; // explicit task override — not a real conflict
        return true;
      });
      if(!culprits.length) return;
      const names=culprits.map(t=>t.name).join(', ');
      warns.push({type:'VACATION_CONFLICT', resId, day, detail:`${res.name} is scheduled on ${sd(day)} during time off: ${names}`});
    });
  });
...
function openEditTask(id){
  ...
  const _durEl=document.getElementById('mt-dur-test'); if(_durEl) _durEl.value=t.timeMode==='test'?(t.dur||1):Math.max(1,Math.round(_testHours/7));
...
window.saveTask=()=>{
  ...
  // For Test tasks, taskTeams entries must store total hours (h/day × dur) not h/day
  const _taskTeamsToSave=tm==='test'
    ? _taskTeams.map(tt=>({...tt,entries:tt.entries.map(e=>({...e,hours:(e.hours||0)*_testDur}))}))
    : _taskTeams;
  const data={name,tags,projId,resId,coResIds,resHours,resource,taskTeams:_taskTeamsToSave,teamId,teamIds,
    group,
...
function renderReport(){
  ...
  // Resolution order: (1) current DOM value if page is active, (2) _rpProjRestore (sync F5),
  // (3) sessionStorage. This handles both F5 restore and the double-call pattern where
  // renderReport() is deferred until loadReportSnapshots() resolves.
  const _rpSel=document.getElementById('rpt-proj-filter');
  let _rpProjId='';
  if(_rpSel){
    // If the page is active and the select already has a valid value, preserve it
    const _rpDomVal=S.page==='report'?_rpSel.value:'';
    let _rpCur=_rpDomVal||window._rpProjRestore||'';
    if(!_rpCur){ try{ _rpCur=JSON.parse(sessionStorage.getItem('pg_ui_'+(S_USER?.resId||''))||'{}')._rpProj||''; }catch(e){} }
    // Rebuild options with selected attribute (works even in hidden elements)
    _rpSel.innerHTML='<option value="">All projects</option>'
      +'<option value="__active__">All active / planning</option>'
      +PROJECTS.map(p=>`<option value="${p.id}"${p.id===_rpCur?' selected':''}>${p.name}</option>`).join('');
    if(_rpCur) _rpSel.value=_rpCur;
    // Use _rpCur directly — _rpSel.value may not reflect selected in hidden elements
    _rpProjId=_rpCur||'';
  }
...
let _lastMetaTs=0; // timestamp of the most recently applied meta snapshot (local or remote)
function _applyMeta(m){
  if(!m) return;
  // PROTECTION 1: never apply a meta snapshot older than the last one we already applied.
  // This guards against out-of-order / delayed / cached SSE events overwriting newer local
  // state with stale ENB/DIS/TASK_DAYS/RES_DAYS data (e.g. a resource's vacation days).
  const _incomingTs=m.ts||0;
  const _isStale=_incomingTs>0 && _lastMetaTs>0 && _incomingTs<_lastMetaTs;
  // PROTECTION 2: if a local 'meta' save is pending (debounced fbFlush not yet sent),
  // don't let a remote snapshot — which predates our local change — overwrite it.
  const _hasPendingMeta = _pendingSave && _pendingSave.meta && _pendingSave.meta.size>0;
  ...
  if(_hasPendingMeta||_isStale) return; // keep local ENB/DIS/TASK_DAYS/RES_DAYS — remote is stale or we have unsaved changes
  if(_incomingTs>0) _lastMetaTs=_incomingTs;
...
async function fbLoad(){
  try{
    ...
    _applyMeta(d.meta);
    // Re-derive RES_DAYS from each resource's timeOff[] — this is the source of truth.
    // meta.RES_DAYS (saved separately) can lag behind or be missing entries due to
    // save-timing races; rebuilding from timeOff[] here guarantees vacations are
    // always respected after a page load, regardless of what was last persisted to /meta.
...
async function fbFlush(){
  ...
  if(snap.meta){ const _metaPatch=_buildMeta(); _lastMetaTs=_metaPatch.ts; sends.push({path:'/meta', patch:_metaPatch}); }
  ...
  es.addEventListener('put', e=>{
    try{
      ...
      if(path==='/'||path===''){
        ...
        // Same as fbLoad(): rebuild RES_DAYS from timeOff[] — source of truth for vacations.
        RESOURCES.forEach(r=>{ if(r.timeOff&&r.timeOff.length) _syncTimeOffToDays(r.id); });
      } else if(path==='/tasks'){
...
<div class="ht-body">
  ...
  <div class="ht-demo">
    <div class="ht-row"><span class="ht-key">group</span><span>Derived automatically from the task name: <b> - 1</b> → Requisitos e Especificação · <b> - 2</b> → Protótipo · <b> - 3</b> → Validação · <b> - 4</b> → Avaliação de Riscos Técnicos · <b> - 5</b> → Handover · <b> - 6</b> → Projeto. If none match, group is left empty.</span></div>
...
function renderProduction(){
  ...
  const search=(()=>{
    const _el=document.getElementById('prod-search');
    if(_el&&!_el.value&&window._savedProdSearch) _el.value=window._savedProdSearch;
    return (_el?.value||'').toLowerCase();
  })();
...
function _mapNewCSVRow(row){
  ...
  return {
    ...
    group: (()=>{
      ...
      if(_n.includes(' - 4')) return 'AVALIAÇÃO DE RISCOS TÉCNICOS';
      if(_n.includes(' - 5')) return 'HANDOVER';
...
function _processCSV(text){
  ...
  // Classify all rows
  const _toUpdate2=rows.filter(r=>{
    const _ex=r.external_id&&TASKS.find(t=>t.externalId===r.external_id);
    if(!_ex) return false;
    // Tasks already done/cancelled: only update if status changes OR logged hours change
    if(_ex.status==='done'||_ex.status==='cancelled'){
      const _statusChanges=r.status!=='done'&&r.status!=='cancelled';
      const _loggedH=(_ex.timeLogs||[]).reduce((s,l)=>s+(l.hours||0),0);
      const _csvSpent=parseFloat(r._rawSpentHours||'0')||0;
      const _loggedChanges=Math.abs(_loggedH-_csvSpent)>0.05;
      if(!_statusChanges&&!_loggedChanges) return false;
    }
    return true;
  });
  ...
  tableEl.innerHTML='<table style="width:100%;border-collapse:collapse"><thead><tr style="background:var(--bg2)">'
    +['Ext ID','Name','Project','Type','Group','Hours','Logged','Resource','Status'].map(h=>`<th style="padding:5px 8px;text-align:left;font-size:10px;color:var(--fg3);font-weight:600;border-bottom:1px solid var(--bd2)">${h}</th>`).join('')
    +'</tr></thead><tbody>'
    +[..._toCreate2,..._toUpdateWithChanges].map((r,i)=>{
      const _ex=TASKS.find(t=>t.externalId&&t.externalId===r.external_id);
      const _changed=field=>_ex&&_hasChanges(r)&&(()=>{
        if(field==='name') return _ex.name!==(r.name||'');
        if(field==='proj'){ const p=PROJECTS.find(p=>p.name.toLowerCase()===(r.project||'').toLowerCase()||(p.orderId||'').split(',').map(s=>s.trim().toLowerCase()).includes((r.project||'').toLowerCase())); return p&&_ex.projId!==p.id; }
        if(field==='group') return _ex.group!==(r.group||'PROTÓTIPO');
        if(field==='hours'){ const _csvH=Math.round((parseFloat(r.total_hours||'0')||0)*100)/100; const _tH=Math.round((tHours(_ex)||0)*100)/100; return Math.abs(_tH-_csvH)>0.05; }
        if(field==='status') return _ex.status!==(r.status||'todo');
        if(field==='resource'){ const nr=(r.resource||'').split(';').map(s=>s.trim()).filter(Boolean).sort((a,b)=>a.toLowerCase().localeCompare(b.toLowerCase())).join(';').toLowerCase(); const or=[_ex.resId,...(_ex.coResIds||[])].filter(Boolean).map(id=>getRes(id)?.name||id).sort((a,b)=>a.toLowerCase().localeCompare(b.toLowerCase())).join(';').toLowerCase(); return nr!==or; }
        return false;
      })();
      const _cell=(field,val)=>`<td style="padding:4px 8px;font-size:10px;color:${_changed(field)?'var(--warn)':'var(--fg2)'};${_changed(field)?'font-weight:600':''}${_changed(field)?';background:rgba(245,158,11,.06)':''}">${val}</td>`;
      const _loggedH=(_ex?.timeLogs||[]).reduce((s,l)=>s+(l.hours||0),0);
      const _csvSpent=parseFloat(r._rawSpentHours||'0')||0;
      const _loggedChanged=_ex&&Math.abs(_loggedH-_csvSpent)>0.05;
      const _loggedCell=`<td style="padding:4px 8px;font-size:10px;color:${_loggedChanged?'var(--warn)':'var(--fg3)'};${_loggedChanged?'font-weight:600;background:rgba(245,158,11,.06)':''}">${_csvSpent>0?_csvSpent+'h':'—'}</td>`;
      const _nameColor=_hasChanges(r)?'var(--warn)':_ex?'var(--fg3)':'var(--fg0)';
      return `<tr style="background:${i%2===0?'transparent':'rgba(255,255,255,.02)'}">
        ...
        <td style="padding:4px 8px;font-size:10px;color:${_nameColor}">${r.name}${_hasChanges(r)?' ✎':_ex?' —':''}</td>
        ${_cell('proj',r.project)}
        ${_cell('type',r.type||r._rawType||'—')}
        ${_cell('group',r.group||'—')}
        ${_cell('hours',(r.total_hours||r.hours_per_day||'—')+'h')}
        ${_loggedCell}
        ${_cell('resource',r.resource||'—')}
        <td style="padding:4px 8px;font-size:10px;font-weight:${_changed('status')?'700':'400'};color:${_changed('status')?'var(--warn)':_ex?(r.status==='done'?'var(--fg3)':r.status==='cancelled'?'var(--fg3)':'var(--fg3)'):r.status==='cancelled'?'var(--danger)':r.status==='done'?'var(--ok)':'var(--fg3)'};${_changed('status')?'background:rgba(245,158,11,.06)':''}">${SLABELS[r.status]||r.status||'To Do'}</td>
      </tr>`;
    }).join('')
    +'</tbody></table>';
...
window.executeCSVImport=()=>{
  ...
  validRows.forEach(row=>{
    ...
    if(_csvTags.includes('teste')){
      ...
      task.timeMode='test';
      const _testeDur=Math.max(1, Math.round(totalHours/7));
      task.hpd=null;
      task.dailyHpd=0;
      ...
      // taskTeams — store total hours (h/day × dur) so modal correctly shows h/day on load
      const _tmTeste={};
      ...
      _coResForTeste.forEach(r=>{ const tid=(r.teams||[])[0]||'_notm'; if(!_tmTeste[tid]) _tmTeste[tid]={teamId:tid==='_notm'?null:tid,entries:[]}; _tmTeste[tid].entries.push({id:r.id,hours:2*_testeDur}); });
...
function _saveSessionState(){
  ...
  try{
    // Load existing saved state so we can preserve values for pages not currently active
    const _sk0='pg_ui_'+S_USER.resId;
    let _prev={};
    try{ _prev=JSON.parse(sessionStorage.getItem(_sk0)||'{}'); }catch(e){}
    // Overview filters: only read from DOM when overview is the active page; otherwise keep previous
    const _ovActive=S.page==='overview';
    const _ovProj  =_ovActive?(document.getElementById('ov-proj')?.value||'')   :(_prev._ovProj  ||'');
    const _ovType  =_ovActive?(document.getElementById('ov-type')?.value||'')   :(_prev._ovType  ||'');
    const _ovSearch=_ovActive?(document.getElementById('ov-search')?.value||'') :(_prev._ovSearch||'');
    // Capture Gantt search and tags
    const _ganttSearch=document.getElementById('g-search')?.value||'';
    const _ganttTags=S.selectedTags||[];
    const _prodSearch=document.getElementById('prod-search')?.value||'';
    // Capture current Project Report filter (preserve when page not active)
    const _rpActive=S.page==='report';
    const _rpProj=_rpActive?(document.getElementById('rpt-proj-filter')?.value||''):(_prev._rpProj||'');
    sessionStorage.setItem('pg_ui_'+S_USER.resId, JSON.stringify({
      page: S.page,
      viewBy: S.viewBy,
      vbFilter: S.vbFilter.filter(x=>x!=='__none__'),
      ...
      _activeProjIds: S._activeProjIds||[],
      _myDayOffset: _myDayOffset||0,
      _ovProj, _ovType, _ovSearch,
      _ganttSearch, _ganttTags,
      _prodSearch,
      _rpProj
    }));
  }catch(e){}
...
function _restoreSessionState(){
  ...
  try{
    ...
    if(saved._activeProjIds?.length) S._activeProjIds=saved._activeProjIds;
    if(saved._myDayOffset) _myDayOffset=saved._myDayOffset;
    if(saved._ganttTags?.length) S.selectedTags=saved._ganttTags;
    if(saved._prodSearch) window._savedProdSearch=saved._prodSearch;
    // Restore Gantt search after DOM is ready
    if(saved._ganttSearch||saved._prodSearch){
      requestAnimationFrame(()=>{
        const _gs=document.getElementById('g-search'); if(_gs&&saved._ganttSearch) _gs.value=saved._ganttSearch;
        const _ps=document.getElementById('prod-search'); if(_ps&&saved._prodSearch) _ps.value=saved._prodSearch;
      });
    }
    // Restore Overview filters — store synchronously so renderOverview() picks them up
    // immediately on F5 (requestAnimationFrame would run AFTER nav() → renderOverview())
    if(saved._ovProj||saved._ovType||saved._ovSearch){
      window._ovProjRestore  = saved._ovProj  || '';
      window._ovTypeRestore  = saved._ovType  || '';
      window._ovSearchRestore= saved._ovSearch|| '';
      // Also apply directly to DOM in case the page is already visible
      requestAnimationFrame(()=>{
        const _op=document.getElementById('ov-proj');   if(_op&&saved._ovProj)   _op.value=saved._ovProj;
        const _ot=document.getElementById('ov-type');   if(_ot&&saved._ovType)   _ot.value=saved._ovType;
        const _os=document.getElementById('ov-search'); if(_os&&saved._ovSearch) _os.value=saved._ovSearch;
      });
    }
    // Restore Project Report filter — store synchronously (same pattern as overview)
    if(saved._rpProj){
      window._rpProjRestore=saved._rpProj;
      requestAnimationFrame(()=>{
        const _rp=document.getElementById('rpt-proj-filter'); if(_rp&&saved._rpProj) _rp.value=saved._rpProj;
      });
    }
    return saved.page;
  }catch(e){ return null; }
...
window.clearProdFilters=()=>{
  const ps=document.getElementById('prod-search'); if(ps) ps.value='';
  const so=document.getElementById('prod-sort'); if(so) so.value='date';
  window._savedProdSearch='';
  _saveSessionState();
  renderProduction();
  notify('Filters cleared','success');
};

window.clearOverviewFilters=()=>{
  const os=document.getElementById('ov-search'); if(os) os.value='';
  const op=document.getElementById('ov-proj'); if(op) op.value='';
  const ot=document.getElementById('ov-type'); if(ot) ot.value='';
  _saveSessionState();
  renderOverview();
  notify('Filters cleared','success');
};
...
window.clearAllFilters=()=>{
  ...
  S.selectedTags=[];
  S.vbFilter=[];
  S._activeProjId=null; S._activeProjIds=[];
  const gs=document.getElementById('g-search'); if(gs) gs.value='';
  const ps=document.getElementById('proj-selector'); if(ps) ps.value='';
...
(async function(){
  ...
  // Mobile starts on My Space, desktop restores saved page or Dashboard
  if(window.innerWidth<=768){
    nav('myspace');
  } else {
    const _savedPage=_restoreSessionState();
    if(_savedPage&&_savedPage!=='dashboard'){
      nav(_savedPage);
    } else {
      nav('dashboard'); renderDash();
    }
  }
